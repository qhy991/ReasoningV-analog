...
Validating this hypothesis, our work adopts a training-free paradigm. Crucially, we challenge the assumption that analog expertise requires domain-specific pre-training. We hypothesize that the reasoning structures required for hardware code generation (HDLs) are \textit{isomorphic} to those needed for analog circuit analysis. Verilog synthesis demands strict causal reasoning and feedback loop analysis, which mirrors the topological dependencies in analog circuits (e.g., LDO stability analysis requires tracing signal propagation through error amplifiers and feedback networks).

Guided by this \textbf{Code-to-Analog Transfer} hypothesis, we leverage ReasoningV-7B---a model optimized for Verilog generation with \textit{no explicit analog training}. \textbf{Surprisingly}, it exhibits remarkable aptitude for analog circuit reasoning. Recent studies suggest that code training instills a generalized ``reasoning scaffold'' into LLMs that transfers to structured tasks \cite{madaan2022language}. We demonstrate that activating this latent scaffold through sophisticated inference strategies is superior to both generic conversational priors and rigid fine-tuned weights. The bottleneck is not the lack of knowledge, but the lack of appropriate context activation \cite{brown2020language}.
...
We selected ReasoningV-7B as our base model not for its domain knowledge (which is digital), but for its \textbf{inference engine}. The model's pre-training on hardware description languages (HDLs) equips it with a strong prior for \textit{structured dependency analysis}---understanding how component A affects component B given condition C. We hypothesize that this capability, rather than raw domain knowledge, is the true bottleneck in analog circuit reasoning. While SFT aligns models for conversational fluency, it often incurs an ``alignment tax'' that degrades pure reasoning capabilities \cite{ouyang2022training}. Traditional analog LLMs acquire domain knowledge through fine-tuning but may lack the systematic reasoning structures that HDL training provides. By using a base reasoning model without aggressive domain-specific SFT, we avoid this degradation and retain the model's latent logical adaptability.
...
\begin{algorithm}[t]
\caption{Taxonomy-Adaptive Strategy Routing}
\label{alg:router}
\SetAlgoLined
\KwIn{Question $Q$, Strategy Set $\mathcal{S}=\{S_{factual}, S_{reasoning}, ...\}$}
\KwOut{Optimized Prompt $P$}
Initialize scores $V \leftarrow \{0, 0, 0, 0, 0\}$\;
Define Keyword Patterns $\mathcal{K} = \{K_{factual}, K_{reasoning}, K_{calc}, K_{analysis}, K_{compare}\}$\;
\ForEach{category $c \in \{factual, reasoning, calc, analysis, compare\}$}{
    $V_c \leftarrow |\text{RegexMatch}(Q, K_c)|$\;
}
$c^* \leftarrow \arg\max_{c} V_c$\;
\eIf{$\max(V) == 0$}{
    $P \leftarrow S_{factual} \oplus Q$ \tcp*{Default fallback}
}{
    $P \leftarrow S_{c^*} \oplus Q$ \tcp*{Concatenate strategy prefix with question}
}
\Return $P$\;
\end{algorithm}
...
\textbf{Router Mechanism Effectiveness:} The Taxonomy-Adaptive Strategy Router successfully identified and applied specialized strategies to 104 out of 1,257 TQA questions. Among the 188 baseline errors, the router-selected strategies corrected 116 errors, achieving a \textbf{61.7\% error correction rate}. Table \ref{tab:router_confusion} presents the router classification confusion matrix on a 200-question validation set, revealing that \textit{calculation-type questions} are most prone to misclassification (83.3\% error rate), primarily confused with factual and reasoning categories.

\begin{table}[htbp]
\caption{Router Classification Confusion Matrix (200 questions)}
\begin{center}
\begin{tabular}{l|ccccc}
\toprule
\textbf{True$\backslash$Pred} & \textbf{Fact.} & \textbf{Reas.} & \textbf{Calc.} & \textbf{Anal.} & \textbf{Comp.} \\
\midrule
Factual & \textbf{95} & 6 & 0 & 3 & 0 \\
Reasoning & 0 & \textbf{69} & 0 & 0 & 0 \\
Calculation & 8 & 12 & \textbf{4} & 0 & 0 \\
Analysis & 0 & 0 & 0 & \textbf{2} & 0 \\
Comparison & 0 & 0 & 0 & 0 & \textbf{1} \\
\bottomrule
\multicolumn{6}{l}{\footnotesize Overall accuracy: 85.50\% (171/200). Calculation→Reasoning: 12 errors.}
\end{tabular}
...
\textbf{Two-Phase Optimization Process:} Our optimization followed a systematic two-phase approach (Table \ref{tab:two_phase}). Phase 1 focused on prompt engineering and parameter optimization, achieving modest gains (+2.37\% overall). Phase 2 introduced few-shot learning and multi-strategy routing, yielding substantial improvements (+5.28\% additional). This demonstrates that while basic optimizations provide a foundation, \textit{few-shot learning is the primary driver of performance gains}.

\begin{table}[htbp]
\caption{Two-Phase Optimization History}
\begin{center}
\begin{tabular}{lcccc}
\toprule
\textbf{Task} & \textbf{Baseline} & \textbf{Phase 1} & \textbf{Phase 2} & \textbf{Total $\Delta$} \\
\midrule
LDO & 46.0\% & 60.0\% & \textbf{81.6\%} & +35.6\% \\
Comparator & 60.0\% & 72.0\% & \textbf{76.0\%} & +16.0\% \\
TQA & 85.0\% & 85.04\% & \textbf{93.32\%} & +8.32\% \\
Caption & 32.5\% & 51.8\% & \textbf{61.27\%} & +28.77\% \\
\midrule
\textbf{Overall} & \textbf{79.01\%} & \textbf{81.38\%} & \textbf{86.66\%} & \textbf{+7.65\%} \\
\bottomrule
\multicolumn{5}{l}{\footnotesize Phase 1: Prompt + Parameter. Phase 2: Few-shot + Router.}
\end{tabular}
\label{tab:two_phase}
\end{center}
\end{table}
...
\begin{itemize}
    ...
    \item Our \textbf{Semantic Router} implements a lightweight reasoning-action loop, conceptually similar to the ReAct framework \cite{react}, allowing the model to dynamically adapt its ``thinking process'' to the circuit topology. Inspired by the Tree of Thoughts framework \cite{yao2023tree}, our Taxonomy-Adaptive Router dynamically selects the optimal reasoning path (e.g., Analytical vs. Factual) based on problem structure, rather than relying on a single generic prompt.
\end{itemize}
...
This aligns with Chen et al.'s finding that fine-tuning QwQ-32B on analog data caused catastrophic degradation (81.54\% $\to$ 74.94\%) \cite{analogseeker}. From a theoretical perspective, Kumar et al. \cite{kumar2022finetuning} demonstrated that fine-tuning can distort the effective pre-trained feature manifold, leading to reduced plasticity when facing novel inference contexts. We hypothesize a three-part mechanism:
...
\subsection{Parametric Knowledge vs. Contextual Reasoning}

Our results illuminate a fundamental trade-off in LLM engineering: \textbf{Parametric Knowledge} (encoded during fine-tuning) versus \textbf{Contextual Reasoning} (activated during inference). This framework explains the divergent behaviors of AnalogSeeker and ReasoningV.

\textbf{AnalogSeeker} exemplifies the parametric approach: domain knowledge is ``baked into'' model weights through SFT on 112M tokens. This yields strong baseline performance (84.97\%) but creates \textit{knowledge rigidity}---the model struggles to adapt when inference-time prompts suggest alternative reasoning patterns. The TQA degradation (-2.38\%) demonstrates this brittleness.

\textbf{ReasoningV} exemplifies the contextual approach: general reasoning capabilities remain flexible, readily shaped by prompts at inference time. This yields lower baseline performance but superior \textit{plasticity}---the model can rapidly adapt to new problem structures via few-shot examples and strategy prefixes.

We term this the \textbf{Knowledge Filler vs. Knowledge Modifier} dichotomy:
\begin{itemize}
    \item For \textit{knowledge-deficient} models (ReasoningV), prompts act as \textbf{knowledge fillers} that bridge domain gaps---few-shot examples become the primary knowledge source, and expert roles activate latent general knowledge.
    \item For \textit{knowledge-rich} fine-tuned models (AnalogSeeker), prompts become \textbf{knowledge modifiers} that may interfere with established parameter manifolds, potentially causing degradation.
\end{itemize}
...
For engineering applications, where problems are inherently dynamic and specifications evolve, we argue that \textbf{contextual reasoning offers a strategic advantage}. A model that can adapt to novel circuit topologies or updated design rules via prompt modification is more valuable than one frozen at a training-time knowledge snapshot. Our results suggest that investing in sophisticated inference strategies may yield better returns than expensive fine-tuning cycles.

\subsection{Does Digital Logic Transfer to Analog Physics?}

A key question arises from our results: \textit{why does a Verilog-optimized model excel at analog tasks?} We argue that Verilog code generation implicitly teaches the model to build \textbf{mental models} of signal propagation and feedback---concepts that are isomorphic to analog circuit operation.

While the underlying physics differ fundamentally (discrete switching vs. continuous voltage/current relationships), the \textbf{reasoning topology} remains consistent: ``Node A affects Node B given Condition C.'' An LDO's error amplifier responding to load transients follows the same logical structure as a Verilog module propagating signals through combinational logic. Figure \ref{fig:isomorphism} illustrates this structural correspondence.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{structural_isomorphism.png}}
\caption{Structural isomorphism between Verilog module hierarchy (left) and LDO circuit topology (right). Both exhibit identical reasoning patterns: signal propagation through functional blocks with feedback loops.}
\label{fig:isomorphism}
\end{figure}

This observation suggests that ``thinking in hardware code'' may be a more effective cognitive bridge to analog design than ``thinking in natural language.'' Natural language models learn correlations between words; HDL-trained models learn \textit{causal dependencies} between system components. For engineering reasoning, where causality is paramount, the latter provides a superior foundation.

Our results empirically validate this hypothesis: ReasoningV (HDL-trained) significantly outperforms AnalogSeeker (fine-tuned on natural language circuit descriptions) when both are equipped with identical prompting strategies. This points toward a new paradigm: rather than fine-tuning on domain text, future analog AI systems might benefit from pre-training on \textit{any} structured reasoning task that emphasizes causal dependencies.
